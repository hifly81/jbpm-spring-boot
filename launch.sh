...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "new-hire-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name new-hire-service apps/new-hire-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app new-hire-service:1.0-SNAPSHOT
    oc expose svc/new-hire-service
else

	echo "Launching the application locally..."
	pattern="new-hire-service"
	files=( $pattern )
	cd ${files[0]}
